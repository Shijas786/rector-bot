...
\`\`\`bash
node -e 'fetch("https://openclaw-predictor-agent-production.up.railway.app/predict-get?telegramId=<ID>&username=<NAME>&claim=" + encodeURIComponent("<CLAIM>")).then(r => r.json()).then(j => console.log(JSON.stringify(j)))'
\`\`\`
...
\`\`\`bash
node -e 'fetch("https://openclaw-predictor-agent-production.up.railway.app/analyse/<SYMBOL>?telegramId=<ID>").then(r => r.json()).then(j => console.log(JSON.stringify(j)))'
\`\`\`
...
2. **ALWAYS** use the `exec` tool with `node -e` for any `production API` call.
