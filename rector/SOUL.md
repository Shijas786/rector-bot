...
- You MUST call the `analyse_token` MCP tool with the token symbol (e.g., "BNB", "BTC").
...
- You MUST call the `record_prediction` MCP tool with their `claim`, `telegramId`, and `username`.
